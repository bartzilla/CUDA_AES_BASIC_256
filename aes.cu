#include <cutil_inline.h>
//#include <aes_test_kernel.cu>
#include "aes_locl.h"
#include <cutil.h>
#include "aes.h"
#include <stdio.h>
#include <stdlib.h>
#include <cuda_runtime_api.h>

static int nGpuArchCoresPerSM[] = { -1, 8, 32 };
   
const int MaxThreadsPerBlock = 512;

extern "C" void ExpandKey(uchar *key, uchar *expkey);

//#define START_VALUE 1024						// 1024 Bytes is the default start value when checking throughputs 
//#define MAX_BYTES_TRANSFER 64 * 512 * 512		// Tests were run in a device with 255393792 bytes of global memory 

static const char *filename = "input.txt";
static const int ITERATIONS = 1;
#define assert(x) if(!(x)){printf ("\nGeneric assert " #x " has thrown an error\n");}//Assert used to compare encryption outputs against NIST Test Vectors

//host variables
unsigned char counter[AES_BLOCK_SIZE]  = {0x00};
unsigned char ecount_buf[AES_BLOCK_SIZE] = {0x00};
uchar expkey[4 * Nb * (Nr + 1)];
double elapsed;

//Device Variables
uchar* d_in = 0;
uchar* d_out = 0;
uchar* d_expkey = 0;
unsigned int* d_length = 0;
uchar* d_counter;
uchar* d_ecount_buf;


// The global data
__device__ __constant__ unsigned char _Sbox[256] = {		// forward s-box
};


__device__ __constant__ unsigned char _InvSbox[256] = {	// inverse s-box
};

// combined Xtimes2[Sbox[]]
__device__ __constant__ unsigned char _Xtime2Sbox[256] = {
 
};

// combined Xtimes3[Sbox[]]
__device__ __constant__ unsigned char _Xtime3Sbox[256] = {
 
};

__device__ __constant__ unsigned char _Xtime2[256] = {
}; 

__device__ __constant__ unsigned char _Xtime9[256] = {
};

__device__ __constant__ unsigned char _XtimeB[256] = {
}; 

__device__ __constant__ unsigned char _XtimeD[256] = {
}; 

__device__ __constant__ unsigned char _XtimeE[256] = {
}; 


// exchanges columns in each of 4 rows
// row0 - unchanged, row1- shifted left 1, 
// row2 - shifted left 2 and row3 - shifted left 3
__device__ void _ShiftRows (uchar *state)
{
	//printf("  - ShiftRows\n");
	uchar tmp;
	
	// just substitute row 0
	state[0] = _Sbox[state[0]], state[4]  = _Sbox[state[4]];
	state[8] = _Sbox[state[8]], state[12] = _Sbox[state[12]];

	// rotate row 1
	tmp = _Sbox[state[1]], state[1] = _Sbox[state[5]];
	state[5] = _Sbox[state[9]], state[9] = _Sbox[state[13]], state[13] = tmp;

	// rotate row 2
	tmp = _Sbox[state[2]], state[2] = _Sbox[state[10]], state[10] = tmp;
	tmp = _Sbox[state[6]], state[6] = _Sbox[state[14]], state[14] = tmp;

	// rotate row 3
	tmp = _Sbox[state[15]], state[15] = _Sbox[state[11]];
	state[11] = _Sbox[state[7]], state[7] = _Sbox[state[3]], state[3] = tmp;
}


// restores columns in each of 4 rows
// row0 - unchanged, row1- shifted right 1, 
// row2 - shifted right 2 and row3 - shifted right 3
__device__ void _InvShiftRows (uchar *state)
{
	//printf("  - InvShiftRows\n");
	uchar tmp;

	// restore row 0
	state[0] = _InvSbox[state[0]], state[4] = _InvSbox[state[4]];
	state[8] = _InvSbox[state[8]], state[12] = _InvSbox[state[12]];

	// restore row 1
	tmp = _InvSbox[state[13]], state[13] = _InvSbox[state[9]];
	state[9] = _InvSbox[state[5]], state[5] = _InvSbox[state[1]], state[1] = tmp;

	// restore row 2
	tmp = _InvSbox[state[2]], state[2] = _InvSbox[state[10]], state[10] = tmp;
	tmp = _InvSbox[state[6]], state[6] = _InvSbox[state[14]], state[14] = tmp;

	// restore row 3
	tmp = _InvSbox[state[3]], state[3] = _InvSbox[state[7]];
	state[7] = _InvSbox[state[11]], state[11] = _InvSbox[state[15]], state[15] = tmp;
}


// recombine and mix each row in a column
__device__ void _MixSubColumns (uchar *state)
{
	//printf("  - MixSubColumns\n");
	uchar tmp[4 * Nb];

	// mixing column 0
	tmp[0] = _Xtime2Sbox[state[0]] ^ _Xtime3Sbox[state[5]] ^ _Sbox[state[10]] ^ _Sbox[state[15]];
	tmp[1] = _Sbox[state[0]] ^ _Xtime2Sbox[state[5]] ^ _Xtime3Sbox[state[10]] ^ _Sbox[state[15]];
	tmp[2] = _Sbox[state[0]] ^ _Sbox[state[5]] ^ _Xtime2Sbox[state[10]] ^ _Xtime3Sbox[state[15]];
	tmp[3] = _Xtime3Sbox[state[0]] ^ _Sbox[state[5]] ^ _Sbox[state[10]] ^ _Xtime2Sbox[state[15]];

	// mixing column 1
	tmp[4] = _Xtime2Sbox[state[4]] ^ _Xtime3Sbox[state[9]] ^ _Sbox[state[14]] ^ _Sbox[state[3]];
	tmp[5] = _Sbox[state[4]] ^ _Xtime2Sbox[state[9]] ^ _Xtime3Sbox[state[14]] ^ _Sbox[state[3]];
	tmp[6] = _Sbox[state[4]] ^ _Sbox[state[9]] ^ _Xtime2Sbox[state[14]] ^ _Xtime3Sbox[state[3]];
	tmp[7] = _Xtime3Sbox[state[4]] ^ _Sbox[state[9]] ^ _Sbox[state[14]] ^ _Xtime2Sbox[state[3]];

	// mixing column 2
	tmp[8] = _Xtime2Sbox[state[8]] ^ _Xtime3Sbox[state[13]] ^ _Sbox[state[2]] ^ _Sbox[state[7]];
	tmp[9] = _Sbox[state[8]] ^ _Xtime2Sbox[state[13]] ^ _Xtime3Sbox[state[2]] ^ _Sbox[state[7]];
	tmp[10]  = _Sbox[state[8]] ^ _Sbox[state[13]] ^ _Xtime2Sbox[state[2]] ^ _Xtime3Sbox[state[7]];
	tmp[11]  = _Xtime3Sbox[state[8]] ^ _Sbox[state[13]] ^ _Sbox[state[2]] ^ _Xtime2Sbox[state[7]];

	// mixing column 3
	tmp[12] = _Xtime2Sbox[state[12]] ^ _Xtime3Sbox[state[1]] ^ _Sbox[state[6]] ^ _Sbox[state[11]];
	tmp[13] = _Sbox[state[12]] ^ _Xtime2Sbox[state[1]] ^ _Xtime3Sbox[state[6]] ^ _Sbox[state[11]];
	tmp[14] = _Sbox[state[12]] ^ _Sbox[state[1]] ^ _Xtime2Sbox[state[6]] ^ _Xtime3Sbox[state[11]];
	tmp[15] = _Xtime3Sbox[state[12]] ^ _Sbox[state[1]] ^ _Sbox[state[6]] ^ _Xtime2Sbox[state[11]];

	for ( int i = 0; i < 4*Nb; ++i )
		state[i] = tmp[i];
}


// restore and un-mix each row in a column
__device__ void _InvMixSubColumns (uchar *state)
{
	//printf("  - InvMixSubColumns\n");
	uchar tmp[4 * Nb];
	int i;

	// restore column 0
	tmp[0] = _XtimeE[state[0]] ^ _XtimeB[state[1]] ^ _XtimeD[state[2]] ^ _Xtime9[state[3]];
	tmp[5] = _Xtime9[state[0]] ^ _XtimeE[state[1]] ^ _XtimeB[state[2]] ^ _XtimeD[state[3]];
	tmp[10] = _XtimeD[state[0]] ^ _Xtime9[state[1]] ^ _XtimeE[state[2]] ^ _XtimeB[state[3]];
	tmp[15] = _XtimeB[state[0]] ^ _XtimeD[state[1]] ^ _Xtime9[state[2]] ^ _XtimeE[state[3]];

	// restore column 1
	tmp[4] = _XtimeE[state[4]] ^ _XtimeB[state[5]] ^ _XtimeD[state[6]] ^ _Xtime9[state[7]];
	tmp[9] = _Xtime9[state[4]] ^ _XtimeE[state[5]] ^ _XtimeB[state[6]] ^ _XtimeD[state[7]];
	tmp[14] = _XtimeD[state[4]] ^ _Xtime9[state[5]] ^ _XtimeE[state[6]] ^ _XtimeB[state[7]];
	tmp[3] = _XtimeB[state[4]] ^ _XtimeD[state[5]] ^ _Xtime9[state[6]] ^ _XtimeE[state[7]];

	// restore column 2
	tmp[8] = _XtimeE[state[8]] ^ _XtimeB[state[9]] ^ _XtimeD[state[10]] ^ _Xtime9[state[11]];
	tmp[13] = _Xtime9[state[8]] ^ _XtimeE[state[9]] ^ _XtimeB[state[10]] ^ _XtimeD[state[11]];
	tmp[2]  = _XtimeD[state[8]] ^ _Xtime9[state[9]] ^ _XtimeE[state[10]] ^ _XtimeB[state[11]];
	tmp[7]  = _XtimeB[state[8]] ^ _XtimeD[state[9]] ^ _Xtime9[state[10]] ^ _XtimeE[state[11]];

	// restore column 3
	tmp[12] = _XtimeE[state[12]] ^ _XtimeB[state[13]] ^ _XtimeD[state[14]] ^ _Xtime9[state[15]];
	tmp[1] = _Xtime9[state[12]] ^ _XtimeE[state[13]] ^ _XtimeB[state[14]] ^ _XtimeD[state[15]];
	tmp[6] = _XtimeD[state[12]] ^ _Xtime9[state[13]] ^ _XtimeE[state[14]] ^ _XtimeB[state[15]];
	tmp[11] = _XtimeB[state[12]] ^ _XtimeD[state[13]] ^ _Xtime9[state[14]] ^ _XtimeE[state[15]];

	for( i=0; i < 4 * Nb; i++ )
		state[i] = _InvSbox[ tmp[i] ];
}

// encrypt/decrypt columns of the key
// n.b. you can replace this with
//      byte-wise xor if you wish.
__device__ void _AddRoundKey (unsigned *state, unsigned *key)
{
	//printf("  - AddRoundKey\n");
	//printf("    - State: "); for ( int __i = 0; __i < 4; ++__i ) //printf("%08x", state[__i]); printf("\n");
	//printf("    - Key:   "); for ( int __i = 0; __i < 4; ++__i ) //printf("%08x", key[__i]); printf("\n");

	for ( int i = 0; i < 4; ++i )
		state[i] ^= key[i];

	//printf("    - State: "); for ( int __i = 0; __i < 4; ++__i ) //printf("%08x", state[__i]); printf("\n");
	//printf("    - Key:   "); for ( int __i = 0; __i < 4; ++__i ) //printf("%08x", key[__i]); printf("\n");
}

// Encrypt one block
__global__ void _AES_ctr128_encrypt (uchar *in, uchar *expkey, uchar *out)
{
	//printf("- Encrypt\n");
	//printf("  - In:  "); dump(16, in); printf("\n");

	__shared__ uchar buffer[16*MaxThreadsPerBlock];

	// Constant data about this thread
	const int block_start = blockIdx.x * blockDim.x * 16;
	const int thread_offset = threadIdx.x * 16;

	// Read in the message data for the whole block
	for ( int i = 0; i < 16; ++i )
	{
		unsigned int index = blockDim.x * i + threadIdx.x;
		buffer[index] = in[block_start + index];
	}
	__syncthreads();

	_AddRoundKey( (unsigned*)(buffer+thread_offset), (unsigned*)expkey );
	for( int round = 1; round < Nr + 1; round++ )
	{
		if( round < Nr )
			_MixSubColumns( buffer+thread_offset );
		else
			_ShiftRows( buffer+thread_offset );

		_AddRoundKey( (unsigned*)(buffer+thread_offset), (unsigned*)expkey + round * Nb );
	}
	__syncthreads();

	// Copy back the memory
	for ( int i = 0; i < 16; ++i )
	{
		unsigned int index = blockDim.x * i + threadIdx.x;
		out[block_start + index] = buffer[index];
	}
}

__host__ double AES_set_encrption(unsigned char* in, unsigned char* out, int length, unsigned char* key)
{
	double elapsed = 0.0;
	uchar expkey[4 * Nb * (Nr + 1)];
	ExpandKey( (uchar*)key, (uchar*)expkey );
	
	// Allocate memory for the GPU
	CUDA_SAFE_CALL( cudaMalloc( (void **) &d_in, length ) );
	CUDA_SAFE_CALL( cudaMalloc( (void **) &d_out, length ) );
	CUDA_SAFE_CALL( cudaMalloc( (void **) &d_expkey, 4 * Nb * (Nr + 1) ) );

	// Copy memory to the GPU
	//CUDA_SAFE_CALL( cudaMemcpy( (void*)d_ecount_buf, (void*)ecount_buf, sizeof(ecount_buf), cudaMemcpyHostToDevice ) );

	CUDA_SAFE_CALL( cudaMemcpy( (void*)d_in, (void*)in, length, cudaMemcpyHostToDevice ) );
	CUDA_SAFE_CALL( cudaMemcpy( (void*)d_expkey, (void*)expkey, 4 * Nb * (Nr + 1), cudaMemcpyHostToDevice ) );

	int units = length / 16;
	int threads = (units >= MaxThreadsPerBlock) ? MaxThreadsPerBlock : (units % MaxThreadsPerBlock);
	int blocks = units/threads;

	unsigned int timer;
	CUT_SAFE_CALL( cutCreateTimer(&timer) );
	cudaThreadSynchronize();
	CUT_SAFE_CALL( cutStartTimer(timer) );
	
	//printf("Debug: Invoking kernel _Encrypt <<<%u,%u>>> (d_in, d_expkey, d_out)\n", blocks, threads);
	_AES_ctr128_encrypt <<<blocks, threads>>> (d_in,d_expkey, d_out);
	//_AES_ctr128_encrypt <<<blocks, threads>>> (d_in,d_expkey, d_out);
	CUT_CHECK_ERROR("Error in kernel.");
	//printf("Debug: Success!\n");

	// IT CRASHES WHEN LENGHT IS 33.554.432 Byte 512 THIS WAS DUE TO BANDWITH WAS NOT SUFFICENT MORE THAN THAT CANNOT BE TRANSFERED IN THIS CARD
	// IT CRASHES WHEN LENGHT IS 8192 BYTES 

	// Stop the timer
	cudaThreadSynchronize();
	CUT_SAFE_CALL( cutStopTimer(timer) );
	elapsed = cutGetTimerValue(timer);
	CUT_SAFE_CALL( cutDeleteTimer(timer) );

	// Copy the results back to the CPU
	CUDA_SAFE_CALL( cudaMemcpy( (void*)out, (void*)d_out, length, cudaMemcpyDeviceToHost ) );

	//printf("    - Out:  "); dump(16, out); //printf("\n");

	CUDA_SAFE_CALL( cudaFree(d_in) );
	CUDA_SAFE_CALL( cudaFree(d_out) );
	CUDA_SAFE_CALL( cudaFree(d_expkey ));

	return elapsed / 1000.0;
}

__host__ void test_vectors ()
{	
	//NIST Test Vectors AES 128
	//The following link contains the test vectors used below to guarantee that the functionality 
	//of the algorithm hast not been modified http://csrc.nist.gov/publications/nistpubs/800-38a/sp800-38a.pdf Block #1 page 24
	
	////AES 128 CTR
	unsigned char testPlainText[]  = {0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96, 0xe9, 0x3d, 0x7e, 0x11, 0x73, 0x93, 0x17, 0x2a};	
	unsigned char testKeyText[] =  {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c};
	unsigned char testCipherText[] = {0x3a, 0xd7, 0x7b, 0xb4, 0x0d, 0x7a, 0x36, 0x60, 0xa8, 0x9e, 0xca, 0xf3, 0x24, 0x66, 0xef, 0x97};
	
	unsigned char out[16] = {0x0};
	
	//Display plaintext to the user
	printf("\n  PlaintText: "); 
	for (int i = 0; i < AES_BLOCK_SIZE; i++)
		printf("%x", testPlainText[i]);
	printf("\n");

	//Display key to the user
	printf("\  Key: "); 
	for (int i = 0; i < AES_BLOCK_SIZE; i++)
		printf("%x", testKeyText[i]);
	printf("\n");

	//Display ciphertext to the user
	printf("\  CipherText: "); 
	for (int i = 0; i < AES_BLOCK_SIZE; i++)
		printf("%x", testCipherText[i]);
	printf("\n");
	
	//AES Encryption
	AES_set_encrption(testPlainText, out, sizeof(testPlainText), testKeyText);


	//Display encrypted data
	printf("\n  GPU Encryption: "); 
	for (int i = 0; i < AES_BLOCK_SIZE; i++)
		printf("%x", out[i]);
	
	//Assert that the encrypted output is the same as the NIST testCipherText vector 
	assert (memcmp (out, testCipherText, 16) == 0);


} 

__host__ double check_throughtputs()
{

	FILE *f_in;
	const int MAX_FILE_SIZE = 64 * 512 * 512;
	const int MAX_BANDWIDTH = 64 * 512 * 512;
	const int START_VALUE = 1024;//16;
	int fileSize =0;

	char *in;	// plaintext  
	char *out;	// ciphertext
	char *tmp;	// Intermediate
	char *filename = "input.txt";
	int block_start;
	int block_offset; 

	u32 ckey[] = {0x00, 0x01, 0x02, 0x03, 0x05, 0x06, 0x07, 0x08, 0x0A, 0x0B, 0x0C, 0x0D, 0x0F, 0x10, 0x11, 0x12};

	int sizeof_key = sizeof(ckey);

	int i, size;
	size_t num_read;
	double base, timer;

	// Read the file into memory
	f_in = fopen (filename, "rb");
	fseek(f_in, 0, SEEK_END);
	fileSize = ftell(f_in); 
	fseek(f_in, 0, SEEK_SET); 

	in  = (char*)calloc (fileSize, sizeof(char));	// plaintext
	out = (char*)calloc (fileSize, sizeof(char));	// ciphertext
	tmp = (char*)calloc (fileSize, sizeof(char));	// Intermediate
	
	assert (f_in != NULL);
	num_read = fread (in, sizeof(char), fileSize, f_in);
	fclose (f_in);

	// Remember how many bytes were read, and round it up to a multiple of 16 to avoid padding My Test Wont give a SHITE about extra bits on the las byte!
	num_read = (int)(ceil (num_read / 16.0) * 16);
	//myAssert (num_read <= MAX_FILE_SIZE);

	
	printf("\n%16s%16s%20s\n", "Total KB", "Seconds", "Throughput Mbps");
	for ( size = START_VALUE; size <= fileSize ; size *= 2 ) 
	{	
		timer = 0.0;

		// Encrypt it X times 
		for ( i = 0; i <1; ++i ){
			//if(size<=MAX_BANDWIDTH){
				/* Encrypt with a 128bit key */  
					//AES_set_encrption(testPlainText, out, sizeof(testPlainText), testKeyText);
					timer +=AES_set_encrption((unsigned char*) in,(unsigned char*) tmp, size,(unsigned char*) ckey);						

/*			}else{
				block_start = 0;
				block_offset = MAX_BANDWIDTH; 
				do{		
					 
					timer +=gpu_encrypt_string((unsigned char*) (in + block_start), MAX_BANDWIDTH, (unsigned char*) ckey, (unsigned char*)(tmp + block_start));
										
					block_start+=MAX_BANDWIDTH; 
					block_offset+=MAX_BANDWIDTH;
					
				}while(block_start<size); 				
			}	*/		
		
		}

		printf ("%16i%16.3f%16.3f\n", size/1024, timer, (size*8.0/(1024*1024))/ timer);
	}

	printf ("\t\n--------------------------------------------------\n");

	free (in);
	free (out);	
	free (tmp);

	return timer;
}
 

__host__  void initCuda() {
	/* Verify the presence of a CUDA supported device */
	cudaError_t cudaerrno;
	int deviceCount;
	int driverVersion = 0;
	cudaGetDeviceCount(&deviceCount);
	cudaerrno = cudaGetLastError();
	if (cudaSuccess != cudaerrno) {
		printf("An error has occur in: '%s' line: %i : %s.\n",__FILE__,__LINE__, cudaGetErrorString(cudaerrno));
		exit(EXIT_FAILURE);
	}
	if (deviceCount == 0) {
		printf("No CUDA supported devices were found.\n");
		exit(EXIT_FAILURE);
	} else {
		printf("CUDA device succesfully found.\n");
	}

	fprintf(stdout,"\n");
}
__host__ void runCipher(){

	printf ("\nAES 128 Bit key");
	printf ("\t\nNIST Test Vectors: ");
	printf ("\t\n--------------------------------------------------");
	test_vectors();
	printf ("\n  Test status: Passed\n");

	printf ("\t\nAES on GPU Throughputs: ");
	printf ("\t\n--------------------------------------------------");
	check_throughtputs();
} 


int main(int argc, char *argv[]){  


  initCuda();
  runCipher();
  
  system("PAUSE");	
  return 0;
} 
